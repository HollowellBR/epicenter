# Steno Architecture Deep Dive

Steno uses a clean three-layer architecture that achieves **extensive code sharing** between the desktop app (Tauri) and web app. This is possible because of how we handle platform differences and separate business logic from UI concerns.
...
The query layer also transforms service-specific errors into UI-friendly error types that integrate seamlessly with the toast notification system. This happens inside `mutationFn` or `queryFn`, creating a clean boundary between business logic errors and UI presentation:
...
if (serviceError) {
  // Query layer transforms to UI-friendly StenoError
  return Err(StenoError({
