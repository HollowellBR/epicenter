	...
	(async () => {
		...
		if (!isAccessibilityGranted) {
			...
			toast.warning('Accessibility Permission Issue', {
				...
				description:
					'Steno needs accessibility permissions. This often requires removing and re-adding the app after updates.',
	...
	(async () => {
		...
		if (!isMicrophoneGranted) {
			...
			toast.info('Microphone Permission Required', {
				...
				description: 'Steno needs microphone access to record audio',
				duration: Number.POSITIVE_INFINITY,
				action: {
