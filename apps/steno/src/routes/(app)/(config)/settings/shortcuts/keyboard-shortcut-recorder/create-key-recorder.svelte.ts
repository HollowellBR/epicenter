 ...
 */
export function createKeyRecorder(options: {
	...
	function completeRecording() {
		...
		options.onRegister(combination);
	}
	...
	$effect(() => {
		...
		if (options.pressedKeys.current.includes('escape')) {
			isListening = false;
			clearCaptureTimer();
			capturedKeys.clear();
			return;
		...
		for (const key of options.pressedKeys.current) {
		...
		if (options.pressedKeys.current.length === 0 && capturedKeys.size > 0) {
			completeRecording();
		}
	});
