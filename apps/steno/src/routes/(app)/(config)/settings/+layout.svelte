<script lang="ts">
	...
	const versionPromise = (async () => {
		const res = await fetch(
			'https://api.github.com/repos/StenoApp/steno/releases/latest',
		);
...
<main class="flex w-full flex-1 flex-col pb-4 pt-2 px-4 sm:px-6 mx-auto max-w-6xl">
	<div
		...
		<div class="space-y-0.5">
			...
			<p class="text-muted-foreground">
				{#await versionPromise}
					Customize your Steno experience.
				{:then v}
					{#if v.isOutdated}
						{@const { latestVersion, currentVersion, latestReleaseUrl } = v}
						Customize your experience for Steno {currentVersion} (latest
						<Link
					...
					{:else}
						{@const { version } = v}
						Customize your experience for Steno {version}.
					{/if}
				{:catch error}
					Customize your Steno experience.
				{/await}
