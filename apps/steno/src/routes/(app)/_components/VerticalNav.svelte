<script lang="ts">
	...
	import { useSidebar } from '@steno/ui/sidebar';
	...
	const navItems = [
		...
		},
		{
			...
			icon: MegaphoneIcon,
			isActive: (pathname: string) =>
				pathname === '/voice-commands' ||
				pathname.startsWith('/voice-commands/'),
		},
		{
			label: 'Settings',
			href: '/settings',
	...
	<Sidebar.Header>
		<Sidebar.Menu>
			<Sidebar.MenuItem>
				...
				>
					{#snippet child({ props })}
						<button {...props} onclick={sidebar.toggle} aria-label="Toggle sidebar">
							<div
							...
							<div class="grid flex-1 text-left text-sm leading-tight">
								<span class="truncate font-semibold">Steno</span>
		...
		<Sidebar.Menu>
			...
			<Sidebar.MenuItem>
				<Sidebar.MenuButton>
					{#snippet child({ props })}
						<a
							href="https://github.com/StenoApp/steno"
							target="_blank"
