/**
 * Centralized path constants for the Steno desktop app.
 *
 * All paths are absolute and resolve relative to the platform-specific app data directory:
 * - macOS: `~/Library/Application Support/io.steno.app/`
 * - Windows: `%APPDATA%/io.steno.app/`
 * - Linux: `~/.config/io.steno.app/`
 *
 * Methods are async because they use Tauri's path APIs which require dynamic imports.
 */
export const PATHS = {
	/** Path to the settings JSON fallback file for crash recovery. */
	async SETTINGS_JSON() {
		const { appDataDir, join } = await import('@tauri-apps/api/path');
		return await join(await appDataDir(), 'settings.json');
	},
	...
	MODELS: {
		...
		/** Directory for local LLM (GGUF) files served by llama.cpp */
		async LLM() {
			const { appDataDir, join } = await import('@tauri-apps/api/path');
			const dir = await appDataDir();
			return await join(dir, 'models', 'llm');
		},
	},
	...
	DB: {
		...
		async VOICE_COMMANDS() {
			const { appDataDir, join } = await import('@tauri-apps/api/path');
			const dir = await appDataDir();
			return await join(dir, 'voice-commands');
		},

		async VOICE_COMMAND_MD(id: string) {
			const { appDataDir, join } = await import('@tauri-apps/api/path');
			const dir = await appDataDir();
			return await join(dir, 'voice-commands', `${id}.md`);
		},
		...
		async SHELL_ALLOWLIST() {
			const { appDataDir, join } = await import('@tauri-apps/api/path');
			const dir = await appDataDir();
			return await join(dir, 'shell-allowlist');
		},

		async SHELL_ALLOWLIST_MD(id: string) {
			const { appDataDir, join } = await import('@tauri-apps/api/path');
			const dir = await appDataDir();
			return await join(dir, 'shell-allowlist', `${id}.md`);
		},
	},
};
