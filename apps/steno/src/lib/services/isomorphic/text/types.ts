...
import type { MaybePromise, StenoError } from '$lib/result';
...
export type TextService = {
	...
	writeToCursor: (
		text: string,
	) => MaybePromise<Result<void, TextServiceError | StenoError>>;
	...
	/**
	 * Simulates an arbitrary keystroke combination.
	 * @param keys Key combination string, e.g. "Ctrl+Shift+S"
	 * @param delayMs Delay in ms between modifier press and main key press
	 */
	simulateKeystroke: (keys: string, delayMs?: number) => Promise<Result<void, TextServiceError>>;
};
