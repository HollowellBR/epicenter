...
import { VoiceCommand } from './models/voice-commands';
import { ShellAllowlistEntry } from './models/shell-allowlist';
	...
	return {
		...
		voiceCommands: {
			async getAll() {
				return tryAsync({
					try: async () => {
						const voiceCommandsPath = await PATHS.DB.VOICE_COMMANDS();
						const dirExists = await exists(voiceCommandsPath);
						if (!dirExists) {
							await mkdir(voiceCommandsPath, { recursive: true });
							return [];
						}
						const contents = await readMarkdownFiles(voiceCommandsPath);
						const commands = contents.map((content) => {
							const { data } = matter(content);
							const validated = VoiceCommand(data);
							if (validated instanceof type.errors) {
								console.error('Invalid voice command:', validated.summary);
								return null;
							}
							return validated;
						});
						return commands.filter((c): c is VoiceCommand => c !== null);
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error getting all voice commands from file system: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			async getById(id: string) {
				return tryAsync({
					try: async () => {
						const mdPath = await PATHS.DB.VOICE_COMMAND_MD(id);
						const fileExists = await exists(mdPath);
						if (!fileExists) return null;
						const content = await readTextFile(mdPath);
						const { data } = matter(content);
						const validated = VoiceCommand(data);
						if (validated instanceof type.errors) {
							throw new Error(`Invalid voice command: ${validated.summary}`);
						}
						return validated;
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error getting voice command by id from file system: ${extractErrorMessage(error)}`,
						}),
				});
			},

			async create(commandOrCommands) {
					...
					try: async () => {
						...
						await Promise.all(
							commands.map(async (command) => {
								const mdContent = matter.stringify('', command);
								const mdPath = await PATHS.DB.VOICE_COMMAND_MD(command.id);
								const tmpPath = `${mdPath}.tmp`;
								await writeTextFile(tmpPath, mdContent);
								await rename(tmpPath, mdPath);
							}),
						...
						DbServiceErr({
							message: `Error creating voice command(s) in file system: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			async update(command) {
				...
				return tryAsync({
					try: async () => {
						const mdPath = await PATHS.DB.VOICE_COMMAND_MD(command.id);
						const mdContent = matter.stringify('', commandWithTimestamp);
						const tmpPath = `${mdPath}.tmp`;
						await writeTextFile(tmpPath, mdContent);
						await rename(tmpPath, mdPath);
						return commandWithTimestamp;
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error updating voice command in file system: ${extractErrorMessage(error)}`,
						}),
				});
			},

			async delete(commandOrCommands) {
				const commands = Array.isArray(commandOrCommands)
					? commandOrCommands
					: [commandOrCommands];
				return tryAsync({
					try: async () => {
						const pathsToDelete = await Promise.all(
							commands.map((c) => PATHS.DB.VOICE_COMMAND_MD(c.id)),
						);
						await bulkDeleteFiles(pathsToDelete);
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error deleting voice command(s) from file system: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			async clear() {
				return tryAsync({
					try: async () => {
						...
						const dirExists = await exists(voiceCommandsPath);
						if (dirExists) {
							await remove(voiceCommandsPath, { recursive: true });
							await mkdir(voiceCommandsPath, { recursive: true });
						}
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error clearing voice commands from file system: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			async getCount() {
				return tryAsync({
					try: async () => {
						const { data: commands, error } = await this.getAll();
						if (error) throw error;
						return commands.length;
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error getting voice commands count from file system: ${extractErrorMessage(error)}`,
						}),
				});
			},
		},
		...
		shellAllowlist: {
			async getAll() {
				return tryAsync({
					try: async () => {
						const shellAllowlistPath = await PATHS.DB.SHELL_ALLOWLIST();
						const dirExists = await exists(shellAllowlistPath);
						if (!dirExists) {
							await mkdir(shellAllowlistPath, { recursive: true });
							return [];
						}
						const contents = await readMarkdownFiles(shellAllowlistPath);
						const entries = contents.map((content) => {
							const { data } = matter(content);
							const validated = ShellAllowlistEntry(data);
							if (validated instanceof type.errors) {
								console.error('Invalid shell allowlist entry:', validated.summary);
								return null;
							}
							return validated;
						});
						return entries.filter((e): e is ShellAllowlistEntry => e !== null);
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error getting all shell allowlist entries from file system: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			async getById(id: string) {
				return tryAsync({
					try: async () => {
						const mdPath = await PATHS.DB.SHELL_ALLOWLIST_MD(id);
						const fileExists = await exists(mdPath);
						if (!fileExists) return null;
						const content = await readTextFile(mdPath);
						const { data } = matter(content);
						const validated = ShellAllowlistEntry(data);
						if (validated instanceof type.errors) {
							throw new Error(`Invalid shell allowlist entry: ${validated.summary}`);
						}
						return validated;
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error getting shell allowlist entry by id from file system: ${extractErrorMessage(error)}`,
						}),
				});
			},

			async create(entryOrEntries) {
					...
					try: async () => {
						...
						await Promise.all(
							entries.map(async (entry) => {
								const mdContent = matter.stringify('', entry);
								const mdPath = await PATHS.DB.SHELL_ALLOWLIST_MD(entry.id);
								const tmpPath = `${mdPath}.tmp`;
								await writeTextFile(tmpPath, mdContent);
								await rename(tmpPath, mdPath);
							}),
						...
						DbServiceErr({
							message: `Error creating shell allowlist entry(ies) in file system: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			async update(entry) {
				...
				return tryAsync({
					try: async () => {
						const mdPath = await PATHS.DB.SHELL_ALLOWLIST_MD(entry.id);
						const mdContent = matter.stringify('', entryWithTimestamp);
						const tmpPath = `${mdPath}.tmp`;
						await writeTextFile(tmpPath, mdContent);
						await rename(tmpPath, mdPath);
						...
						DbServiceErr({
							message: `Error updating shell allowlist entry in file system: ${extractErrorMessage(error)}`,
						}),
				});
			},

			async delete(entryOrEntries) {
				const entries = Array.isArray(entryOrEntries)
					? entryOrEntries
					: [entryOrEntries];
				return tryAsync({
					try: async () => {
						const pathsToDelete = await Promise.all(
							entries.map((e) => PATHS.DB.SHELL_ALLOWLIST_MD(e.id)),
						);
						await bulkDeleteFiles(pathsToDelete);
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error deleting shell allowlist entry(ies) from file system: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			async clear() {
				return tryAsync({
					try: async () => {
						...
						const dirExists = await exists(shellAllowlistPath);
						if (dirExists) {
							await remove(shellAllowlistPath, { recursive: true });
							await mkdir(shellAllowlistPath, { recursive: true });
						}
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error clearing shell allowlist from file system: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			async getCount() {
				return tryAsync({
					try: async () => {
						const { data: entries, error } = await this.getAll();
						if (error) throw error;
						return entries.length;
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error getting shell allowlist count from file system: ${extractErrorMessage(error)}`,
						}),
				});
			},
		},
