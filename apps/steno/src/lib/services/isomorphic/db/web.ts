...
import { ToastServiceLive as toast } from '$lib/services/isomorphic/toast';
...
import type { VoiceCommand } from './models/voice-commands';
import type { ShellAllowlistEntry } from './models/shell-allowlist';
import type { DbService } from './types';
import { DbServiceErr } from './types';

const DB_NAME = 'RecordingDB';

class StenoDatabase extends Dexie {
	...
	voiceCommands!: Dexie.Table<VoiceCommand, string>;
	shellAllowlist!: Dexie.Table<ShellAllowlistEntry, string>;
	...
	constructor({ DownloadService }: { DownloadService: DownloadService }) {
		...
		// V7: Add voice commands and shell allowlist tables
		this.version(0.7).stores({
			recordings: '&id, timestamp, createdAt, updatedAt',
			transformations: '&id, createdAt, updatedAt',
			transformationRuns: '&id, transformationId, recordingId, startedAt',
			voiceCommands: '&id, createdAt, updatedAt',
			shellAllowlist: '&id, createdAt, updatedAt',
		});
	}
}
...
export function createDbServiceWeb({
	...
	const db = new StenoDatabase({ DownloadService });
	return {
		recordings: {
			getAll: async () => {
				return tryAsync({
		...
		voiceCommands: {
			getAll: async () => {
				return tryAsync({
					try: () => db.voiceCommands.toArray(),
					catch: (error) =>
						DbServiceErr({
							message: `Error getting all voice commands from Dexie: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			getById: async (id) => {
				return tryAsync({
					try: async () => {
						const result = (await db.voiceCommands.get(id)) ?? null;
						return result;
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error getting voice command by id from Dexie: ${extractErrorMessage(error)}`,
						}),
				});
			},

			create: async (commandOrCommands) => {
				const commands = Array.isArray(commandOrCommands)
					? commandOrCommands
					: [commandOrCommands];
				return tryAsync({
					try: async () => {
						await db.voiceCommands.bulkAdd(commands);
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error adding voice command(s) to Dexie: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			update: async (command) => {
				...
				const { error: updateError } = await tryAsync({
					try: () => db.voiceCommands.put(commandWithTimestamp),
					catch: (error) =>
						DbServiceErr({
							message: `Error updating voice command in Dexie: ${extractErrorMessage(error)}`,
						}),
				});
				if (updateError) return Err(updateError);
				return Ok(commandWithTimestamp);
			},

			delete: async (commandOrCommands) => {
				const commands = Array.isArray(commandOrCommands)
					? commandOrCommands
					: [commandOrCommands];
				return tryAsync({
					try: async () => {
						...
						await db.voiceCommands.bulkDelete(ids);
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error deleting voice command(s) from Dexie: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			clear: async () => {
				return tryAsync({
					try: () => db.voiceCommands.clear(),
					catch: (error) =>
						DbServiceErr({
							message: `Error clearing voice commands from Dexie: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			getCount: async () => {
				return tryAsync({
					try: () => db.voiceCommands.count(),
					catch: (error) =>
						DbServiceErr({
							message: `Error getting voice commands count from Dexie: ${extractErrorMessage(error)}`,
						}),
				});
			},
		},
		...
		shellAllowlist: {
			getAll: async () => {
				return tryAsync({
					try: () => db.shellAllowlist.toArray(),
					catch: (error) =>
						DbServiceErr({
							message: `Error getting all shell allowlist entries from Dexie: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			getById: async (id) => {
				return tryAsync({
					try: async () => {
						const result = (await db.shellAllowlist.get(id)) ?? null;
						return result;
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error getting shell allowlist entry by id from Dexie: ${extractErrorMessage(error)}`,
						}),
				});
			},

			create: async (entryOrEntries) => {
				const entries = Array.isArray(entryOrEntries)
					? entryOrEntries
					: [entryOrEntries];
				return tryAsync({
					try: async () => {
						await db.shellAllowlist.bulkAdd(entries);
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error adding shell allowlist entry(ies) to Dexie: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			update: async (entry) => {
				...
				const entryWithTimestamp = {
					...entry,
					updatedAt: now,
				} satisfies ShellAllowlistEntry;
				const { error: updateError } = await tryAsync({
					try: () => db.shellAllowlist.put(entryWithTimestamp),
					catch: (error) =>
						DbServiceErr({
							message: `Error updating shell allowlist entry in Dexie: ${extractErrorMessage(error)}`,
						}),
				});
				if (updateError) return Err(updateError);
				return Ok(entryWithTimestamp);
			},

			delete: async (entryOrEntries) => {
				const entries = Array.isArray(entryOrEntries)
					? entryOrEntries
					: [entryOrEntries];
				return tryAsync({
					try: async () => {
						...
						await db.shellAllowlist.bulkDelete(ids);
					},
					catch: (error) =>
						DbServiceErr({
							message: `Error deleting shell allowlist entry(ies) from Dexie: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			clear: async () => {
				return tryAsync({
					try: () => db.shellAllowlist.clear(),
					catch: (error) =>
						DbServiceErr({
							message: `Error clearing shell allowlist from Dexie: ${extractErrorMessage(error)}`,
						}),
				});
			},
			...
			getCount: async () => {
				return tryAsync({
					try: () => db.shellAllowlist.count(),
					catch: (error) =>
						DbServiceErr({
							message: `Error getting shell allowlist count from Dexie: ${extractErrorMessage(error)}`,
						}),
				});
			},
		},
