...
export function createDbServiceDesktop({
	...
	return {
		...
		voiceCommands: {
			getAll: async () => {
				const [fsResult, idbResult] = await Promise.all([
					fileSystemDb.voiceCommands.getAll(),
					indexedDb.voiceCommands.getAll(),
				]);
				if (fsResult.error && idbResult.error) {
					return Err({
						name: 'DbServiceError' as const,
						message: 'Error getting all voice commands from both sources',
					});
				}
				const fsCommands = fsResult.data ?? [];
				const idbCommands = idbResult.data ?? [];
				const merged = new Map();
				for (const c of idbCommands) merged.set(c.id, c);
				for (const c of fsCommands) merged.set(c.id, c);
				return Ok(Array.from(merged.values()));
			},

			getById: async (id: string) => {
				const fsResult = await fileSystemDb.voiceCommands.getById(id);
				if (fsResult.data) return Ok(fsResult.data);
				const idbResult = await indexedDb.voiceCommands.getById(id);
				if (idbResult.data) return Ok(idbResult.data);
				if (fsResult.error && idbResult.error) {
					return Err({
						name: 'DbServiceError' as const,
						message: 'Error getting voice command by id from both sources',
					});
				}
				return Ok(null);
			},
			...
			create: async (commandOrCommands) => {
				return fileSystemDb.voiceCommands.create(commandOrCommands);
			},

			update: async (command) => {
				return fileSystemDb.voiceCommands.update(command);
			},

			delete: async (commandOrCommands) => {
				const [fsResult, idbResult] = await Promise.all([
					fileSystemDb.voiceCommands.delete(commandOrCommands),
					indexedDb.voiceCommands.delete(commandOrCommands),
				]);
				if (fsResult.error && idbResult.error) {
					return Err({
						name: 'DbServiceError' as const,
						message: 'Error deleting voice command(s) from both sources',
					});
				}
				return Ok(undefined);
			},
			...
			clear: async () => {
				const [fsResult, idbResult] = await Promise.all([
					fileSystemDb.voiceCommands.clear(),
					indexedDb.voiceCommands.clear(),
				]);
				if (fsResult.error && idbResult.error) {
					return Err({
						name: 'DbServiceError' as const,
						message: 'Error clearing voice commands from both sources',
					});
				}
				return Ok(undefined);
			},
			...
			getCount: async () => {
				return fileSystemDb.voiceCommands.getCount();
			},
		},
		...
		shellAllowlist: {
			getAll: async () => {
				const [fsResult, idbResult] = await Promise.all([
					fileSystemDb.shellAllowlist.getAll(),
					indexedDb.shellAllowlist.getAll(),
				]);
				if (fsResult.error && idbResult.error) {
					return Err({
						name: 'DbServiceError' as const,
						message: 'Error getting all shell allowlist entries from both sources',
					});
				}
				const fsEntries = fsResult.data ?? [];
				const idbEntries = idbResult.data ?? [];
				const merged = new Map();
				for (const e of idbEntries) merged.set(e.id, e);
				for (const e of fsEntries) merged.set(e.id, e);
				return Ok(Array.from(merged.values()));
			},

			getById: async (id: string) => {
				const fsResult = await fileSystemDb.shellAllowlist.getById(id);
				if (fsResult.data) return Ok(fsResult.data);
				const idbResult = await indexedDb.shellAllowlist.getById(id);
				if (idbResult.data) return Ok(idbResult.data);
				if (fsResult.error && idbResult.error) {
					return Err({
						name: 'DbServiceError' as const,
						message: 'Error getting shell allowlist entry by id from both sources',
					});
				}
				return Ok(null);
			},
			...
			create: async (entryOrEntries) => {
				return fileSystemDb.shellAllowlist.create(entryOrEntries);
			},

			update: async (entry) => {
				return fileSystemDb.shellAllowlist.update(entry);
			},

			delete: async (entryOrEntries) => {
				const [fsResult, idbResult] = await Promise.all([
					fileSystemDb.shellAllowlist.delete(entryOrEntries),
					indexedDb.shellAllowlist.delete(entryOrEntries),
				]);
				if (fsResult.error && idbResult.error) {
					return Err({
						name: 'DbServiceError' as const,
						message: 'Error deleting shell allowlist entry(ies) from both sources',
					});
				}
				return Ok(undefined);
			},
			...
			clear: async () => {
				const [fsResult, idbResult] = await Promise.all([
					fileSystemDb.shellAllowlist.clear(),
					indexedDb.shellAllowlist.clear(),
				]);
				if (fsResult.error && idbResult.error) {
					return Err({
						name: 'DbServiceError' as const,
						message: 'Error clearing shell allowlist from both sources',
					});
				}
				return Ok(undefined);
			},
			...
			getCount: async () => {
				return fileSystemDb.shellAllowlist.getCount();
			},
		},
	};
}
