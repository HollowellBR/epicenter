...
This platform abstraction enables **97% code sharing** between Steno's desktop and web versions. The vast majority of application logic is platform-agnostic, with only the thin service implementation layer varying between platforms. Instead of maintaining separate codebases, we write business logic once and let services handle platform differences automatically.
...
3. **UI Layer**: Displays `StenoError` objects in toasts without re-wrapping
...
Services should **never** import or use `StenoError`. That transformation happens in the query layer:

```typescript
// ❌ WRONG - Service shouldn't know about StenoError
import { StenoError } from '$lib/result';

// ✅ CORRECT - Service uses its own error type
type MyServiceError = TaggedError<'MyServiceError'>;
```

The query layer is responsible for transforming service errors into `StenoError` for toast notifications. This separation ensures:
...
Never wrap an already-wrapped error. The query layer handles the single transformation from service error to `StenoError`:
...
if (error) {
	...
	notify.error.execute({ ...stenoError.error }); // Double wrapping!
}

// ✅ GOOD: Service returns tagged error, query wraps it, UI uses directly
if (error) {
	notify.error.execute(error); // Already a StenoError from query layer
}
```
