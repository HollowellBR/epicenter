 ...
 * Enables/disables launching Steno on system login.
 *
...
export const AutostartServiceLive = {
	/** Check if autostart is currently enabled for Steno. */
	isEnabled: () =>
		tryAsync({
			try: () => isEnabled(),
			catch: (error) =>
	...
	/** Enable autostart so Steno launches on system login. */
	enable: () =>
		tryAsync({
			try: () => enable(),
			catch: (error) =>
	...
	/** Disable autostart so Steno does not launch on system login. */
	disable: () =>
		tryAsync({
			try: () => disable(),
			catch: (error) =>
