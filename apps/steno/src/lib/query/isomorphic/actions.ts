...
import { tryIntercept } from './voice-command-interceptor';
...
export const commands = {
	...
	// Open transformation picker (transform-clipboard window removed)
...
async function processRecordingPipeline({
	...
	if (transcribeError) {
		...
		if (transcribeError.name === 'StenoError') {
	...
	// Transcription succeeded - check for voice commands before delivering
	if (settings.value['voiceCommands.enabled']) {
		...
		if (intercepted) {
			// Command was executed - still save the recording but skip text delivery
			const { error: saveError } = await savePromise;
			if (!saveError) {
				await db.recordings.update({
					...recording,
