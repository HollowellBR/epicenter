...
export const transformer = {
	transformInput: defineMutation({
		...
		}): Promise<StenoResult<string>> => {
			const getTransformationOutput = async (): Promise<
				Result<string, StenoError>
			> => {
...
async function handleStep({
	...
	switch (step.type) {
		...
		case 'prompt_transform': {
			...
			const userTemplate =
				step['prompt_transform.userPromptTemplate'] || '{{input}}';

			const systemPrompt = interpolateTemplate(
				asTemplateString(systemTemplate),
				{ input },
			);
			const userPrompt = interpolateTemplate(
				asTemplateString(userTemplate),
				{ input },
			);
			...
			if (provider === 'llamacpp') {
				...
				const modelPath = settings.value['llamacpp.modelPath'];
				const port = settings.value['llamacpp.port'];

				if (!modelPath) {
					return Err(
						'No local model selected. Download a Qwen3 model in Settings → Transformation, or switch the completion provider to Ollama.',
					);
				}

				// Lazily ensure the llama-server sidecar is running (idempotent).
				const { error: startError } = await tryAsync({
					try: () =>
						invoke('start_llama_server', {
							...
							modelPath,
							port,
							ctxSize: settings.value['llamacpp.contextSize'],
							nGpuLayers: settings.value['llamacpp.gpuLayers'],
						}),
					catch: (error) => Err(extractErrorMessage(error)),
				});
				if (startError) return Err(startError);

				return services.completions.LlamaCppCompletionServiceLive.complete({
			...
			const model =
				...
				settings.value['ollama.model'];
			const effectiveBaseUrl =
				step['prompt_transform.inference.provider.Custom.baseUrl'] ||
				settings.value['ollama.baseUrl'];

			return services.completions.OllamaCompletionServiceLive.complete({
