...
A critical responsibility of the query layer is transforming service-specific errors into `StenoError` types that work seamlessly with our toast notification system. This transformation happens inside `mutationFn` or `queryFn`, creating a clean boundary between business logic errors and UI presentation.
...
2. **Query Layer**: Wraps service errors into `StenoError` objects
3. **UI Layer**: Uses `StenoError` directly without re-wrapping

This pattern ensures consistent error handling and avoids double-wrapping errors.

### How It Works

Services return their own specific error types (e.g., `RecorderServiceError`, `CpalRecorderServiceError`), which contain detailed error information. The query layer transforms these into `StenoError` with UI-friendly formatting:
...
startRecording: defineMutation({
	mutationFn: async ({ toastId }: { toastId: string }) => {
		...
		// Transform service error to StenoError
	...
	// StenoError is now available in onError hook
	onError: (error) => {
		// error is StenoError, ready for toast display
...
2. **Query Layer**: Transforms to `StenoError` in `mutationFn`/`queryFn`
...
3. **UI Layer**: Receives `StenoError` in hooks, perfect for toasts
   ```typescript
   onError: (error) => notify.error.execute(error); // error is StenoError
   ```
...
getRecorderState: defineQuery({
	queryFn: async () => {
		...
		if (getRecorderStateError) {
			// Transform CpalRecorderServiceError → StenoError
			return Err(
				StenoError({
...
```typescript
// BAD: Don't wrap an already-wrapped StenoError
if (getRecorderStateError) {
	const stenoError = StenoErr({
		title: '❌ Failed to get recorder state',
		description: getRecorderStateError.message,
		action: { type: 'more-details', error: getRecorderStateError },
	});
	notify.error.execute({ id: nanoid(), ...stenoError.error });
	return stenoError;
}
```
...
getRecorderState: defineQuery({
	...
	initialData: 'IDLE' as StenoRecordingState,
});
```
...
// In UI/command layer - use StenoError directly
if (error) {
...
- Toast notifications receive properly formatted `StenoError` objects
