...
- [x] Transform service errors to StenoError at query layer
...
3. **Transformed Errors at Query Layer**
   ...
   - Convert service-specific errors to StenoError with proper formatting
   - Applied destructuring pattern for cleaner code (e.g., `{ data: blob, error: stopRecordingError }`)

4. **Maintained VAD Service Pattern**
   - VAD service already uses StenoError directly, which is the desired pattern
...
- Clean separation between service layer (TaggedError) and UI layer (StenoError)
